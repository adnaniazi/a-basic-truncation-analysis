extract_read_data_hdf5r <- function(read_path){
    ...
    tombo_normalization_key <- 'Analyses/RawGenomeCorrected_000/BaseCalled_template'
    scale <- f5_obj[[tombo_normalization_key]]$attr_open('scale')$read()
    shift <- f5_obj[[tombo_normalization_key]]$attr_open('shift')$read()
